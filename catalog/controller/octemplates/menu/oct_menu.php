...
class ControllerOCTemplatesMenuOCTMenu extends Controller {
    public function index() {
        ...
        if (isset($oct_showcase_data['megamenu']['status']) && $oct_showcase_data['megamenu']['status']) {
			...
			if ($data['oct_detect'] && isset($oct_showcase_data['mobile_menu']['st_cats']) && $oct_showcase_data['mobile_menu']['st_cats']) {
				$oct_categories = $this->getStandartCategories($data);
			} elseif (!$data['oct_detect'] && isset($oct_showcase_data['megamenu']['dcategories']) && $oct_showcase_data['megamenu']['dcategories']) {
				$oct_categories = $this->getStandartCategories($data);
			} else {
				$oct_categories = [];
			}
    ...
    private function getOCTMegaMenu($data = []) {
        ...
        if (!$menu_items) {
            ...
            $types = [
                'category' => ($data['oct_detect'] && !isset($data['oct_showcase_data']['mobile_menu']['t_cats'])) ? 0 : 1,
                'manufacturer' => ($data['oct_detect'] && !isset($data['oct_showcase_data']['mobile_menu']['brands'])) ? 0 : 1,
                'oct_blogcategory' => ($data['oct_detect'] && !isset($data['oct_showcase_data']['mobile_menu']['blog'])) ? 0 : 1,
                'link' => ($data['oct_detect'] && !isset($data['oct_showcase_data']['mobile_menu']['links'])) ? 0 : 1
            ];
    ...
    private function getStandartCategories($data = []) {
        ...
        $categories_icon = ((isset($oct_showcase_data['mobile_menu']['icon']) && isset($data['oct_detect']) && $data['oct_detect']) || isset($oct_showcase_data['megamenu']['icon'])) ? true : false;
