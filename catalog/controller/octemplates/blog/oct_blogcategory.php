...
class ControllerOCTemplatesBlogOCTBlogCategory extends Controller {
	public function index() {
		...
		if ($blog_category_info) {
            ...
            $data['oct_lazy_image'] = $this->model_tool_image->resize($this->config->get('theme_oct_showcase_lazyload_image') ? $this->config->get('theme_oct_showcase_lazyload_image') : 'catalog/showcase/lazy-image.svg', 30, 30);
			...
			*/
			...
			foreach ($results as $result) {
				...
				// Get categories 
				$blog_category_badge = [];
				$blog_category_info_badge = [];
				$category = $this->model_octemplates_blog_oct_blogarticle->getArticleBlogCategories($result['blogarticle_id']);
				
				foreach ($category as $categories) {

					if (isset($categories['blogcategory_id']) && $categories['blogcategory_id']) {
						$blog_category_info_badge[] = $this->model_octemplates_blog_oct_blogcategory->getBlogCategory($categories['blogcategory_id']);
					}
				}
				
				foreach ($blog_category_info_badge as $blog_name) {
					$blog_category_badge[] = $blog_name['name'];
				}
				...
				$data['articles'][] = [
					...
					'name'					=> $result['name'],
					"description"			=> mb_strimwidth($result["description"], 0, 350, "..."),
